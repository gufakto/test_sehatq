...
ml_bronze = "/opt/airflow/dags/bronze/ml.csv"

ml_silver = "/opt/airflow/dags/silvers/ml.csv"

ml_table = "ml_fact"

with DAG(dag_id="ml_fact", schedule_interval=None, start_date=pendulum.datetime(2022, 1, 1, tz="Asia/Jakarta"), catchup=False, tags=["sehatq"]) as dag:
    def extract(**kwargs):
        ml_query = """WITH RENTAL_FEATURES(film_id,avg_rental_cost,avg_hours_rented)
AS (
    SELECT 
        i.film_id,
...
SELECT * FROM FEATURES_AND_LABELS"""
        table_to_csv(ml_query, ml_bronze)

    def transform(**kwargs):
        df_ml = csv_to_df(ml_bronze)
        df_ml.to_csv(ml_silver, index=False)

    def load(**kwargs):
        csv_to_table(ml_table, ml_silver)
